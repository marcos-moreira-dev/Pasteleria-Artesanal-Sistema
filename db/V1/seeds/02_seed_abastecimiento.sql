-- Seeds CORREGIDOS para Abastecimiento y operaciones
...
-- 6. RECETAS (con campos correctos: es_activa, rendimiento_base, costo_estimado, nombre)
INSERT INTO receta (producto_id, nombre, es_activa, rendimiento_base, costo_estimado, observaciones, created_at, updated_at, version)
SELECT 
    p.producto_id,
    'Receta ' || p.nombre,
    true,
    CASE 
        WHEN p.codigo LIKE 'TORT%' THEN 1
        WHEN p.codigo LIKE 'CUPK%' THEN 12
        WHEN p.codigo LIKE 'MESA%' THEN 25
        ELSE 1
    END,
    p.precio_base * 0.6,
    'Receta estándar',
...
-- 9. PEDIDOS (con estado válido según check constraint)
...
-- 11. COTIZACIONES (sin fecha_cotizacion, usa created_at)
