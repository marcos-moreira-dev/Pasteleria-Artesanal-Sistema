...
@Component({
    ...
    <div class="admin-shell">
      <aside class="admin-sidebar">
        <div class="sidebar-top">
          <section class="brand-panel">
            <div class="brand-lockup">
              <img src="assets/branding/logo-placeholder.svg" alt="Logo placeholder de la pastelería" width="56" height="56" />
              <div>
                <p class="brand-kicker">Pastelería artesanal</p>
                <h1 class="brand-title">Casa de Producción</h1>
              </div>
            </div>
            <p class="brand-copy">Clientes, pedidos, vitrinas y cocina coordinados para que cada entrega salga a tiempo.</p>
          </section>
          ...
          <nav class="admin-nav">
            ...
            <a routerLink="/clientes" routerLinkActive="is-active">
              <img src="assets/icons/clients.svg" alt="" width="20" height="20" aria-hidden="true" />
              <span>Clientes</span>
            </a>
            <a routerLink="/productos" routerLinkActive="is-active">
              <img src="assets/icons/products.svg" alt="" width="20" height="20" aria-hidden="true" />
              <span>Productos</span>
            </a>
            <a routerLink="/cotizaciones" routerLinkActive="is-active">
              <img src="assets/icons/quotations.svg" alt="" width="20" height="20" aria-hidden="true" />
              <span>Cotizaciones</span>
            </a>
            <a routerLink="/reportes" routerLinkActive="is-active">
              <img src="assets/icons/reports.svg" alt="" width="20" height="20" aria-hidden="true" />
              <span>Reportes</span>
            </a>
            <a routerLink="/pedidos" routerLinkActive="is-active">
              <img src="assets/icons/orders.svg" alt="" width="20" height="20" aria-hidden="true" />
              <span>Pedidos</span>
            </a>
            <a routerLink="/produccion" routerLinkActive="is-active">
              <img src="assets/icons/production.svg" alt="" width="20" height="20" aria-hidden="true" />
        ...
        <section class="session-card">
          <div class="session-heading">
            <img src="assets/icons/session.svg" alt="" width="22" height="22" aria-hidden="true" />
      ...
      <main class="admin-main">
        <header class="admin-header">
          <div>
            <p class="header-eyebrow">Jornada del día</p>
            <h2>{{ pageTitle() }}</h2>
          </div>
          <div class="header-actions">
            <div class="notification-shell">
              <button type="button" class="notification-button" (click)="toggleNotifications()">
                <span class="refresh-button__icon">
                  <img src="assets/icons/notifications.svg" alt="" width="18" height="18" aria-hidden="true" />
                </span>
                <span>Notificaciones</span>
                <strong *ngIf="shellFacade.notificationCounters().unread > 0" class="notification-badge">
              ...
              <section class="notification-panel" *ngIf="notificationsOpen()">
                <header>
                  ...
                  <strong>{{ shellFacade.notificationCounters().unread }} sin leer</strong>
                </header>

                <div class="notification-actions" *ngIf="shellFacade.notifications().length">
                  <label class="selection-toggle">
                    <input
                      type="checkbox"
                      [checked]="allVisibleNotificationsSelected()"
                      (change)="toggleAllVisibleNotifications($any($event.target).checked)"
                    />
                  ...
                  <div class="action-row">
                    ...
                    <button type="button" class="mini-button mini-button--ghost" (click)="archiveReadNotifications()">
                      Ocultar leídas
                    </button>
                  </div>
                </div>

                <div class="notification-list" *ngIf="shellFacade.notifications().length; else emptyNotifications">
                  <article *ngFor="let notification of shellFacade.notifications()">
                    <div class="notification-row">
                      <label class="selection-check" aria-label="Seleccionar notificación">
                        <input
                          type="checkbox"
                          [checked]="isNotificationSelected(notification.id)"
                          (change)="toggleNotificationSelection(notification.id, $any($event.target).checked)"
                        />
                      </label>
                      <p>{{ notification.title }}</p>
                      <span class="status-pill">{{ prettyNotificationPriority(notification.priority) }}</span>
                    </div>
                    <small>{{ notification.module }} · {{ notification.createdAt | date:'short' }}</small>
                ...
                <ng-template #emptyNotifications>
                  <p class="notification-empty">Todavía no hay notificaciones para esta sesión.</p>
                </ng-template>
              </section>
            </div>
            ...
            <button type="button" class="refresh-button" (click)="reload()">
              <span class="refresh-button__icon">
                <img src="assets/icons/refresh.svg" alt="" width="18" height="18" aria-hidden="true" />
              </span>
        ...
        <section class="status-banner" *ngIf="shellFacade.loading()">Actualizando pedidos, clientes y producción...</section>
        <section class="status-banner is-success" *ngIf="shellFacade.actionMessage()">{{ shellFacade.actionMessage() }}</section>
        <section class="status-banner is-error" *ngIf="shellFacade.error()">{{ shellFacade.error() }}</section>
  ...
  styles: [`
    ...
    .admin-sidebar {
      ...
      background: linear-gradient(180deg, #462019 0%, #5c2c19 38%, #2b140e 100%);
      color: #fff2eb;
    ...
    .brand-title {
      margin: 0.5rem 0 0;
      font: 700 2rem/1.05 var(--font-display, "Cormorant Garamond", Georgia, serif);
    }
    ...
    .admin-nav a img {
      display: block;
      filter: brightness(0) saturate(100%) invert(94%) sepia(22%) saturate(428%) hue-rotate(309deg) brightness(102%) contrast(92%);
      opacity: 0.95;
    }
    ...
    .admin-main {
      ...
      padding: 1.8rem;
      overflow-x: hidden;
      background: radial-gradient(circle at top left, #fff2e8 0%, #f6efe9 30%, #f3f4f6 75%);
    }
    ...
    .admin-header h2 {
      margin: 0.35rem 0 0;
      font: 700 1.8rem/1.1 var(--font-display, "Cormorant Garamond", Georgia, serif);
      color: #2d201a;
    }
    ...
    .notification-panel h3 {
      margin: 0.3rem 0 0;
      font: 700 1.1rem/1.1 var(--font-display, "Cormorant Garamond", Georgia, serif);
      color: #2d201a;
    }
...
export class ShellComponent {
  ...
  readonly allVisibleNotificationsSelected = computed(() => {
    const visibleIds = this.shellFacade.notifications().map((notification) => notification.id);
    return visibleIds.length > 0 && visibleIds.every((notificationId) => this.selectedNotificationIds().includes(notificationId));
  });
  ...
  markNotificationAsRead(notificationId: number) {
    ...
    this.selectedNotificationIds.update((current) => current.filter((id) => id !== notificationId));
  }
