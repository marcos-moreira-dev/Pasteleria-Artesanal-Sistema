  ...
  template: `
    <section class="surface-card">
      <header class="surface-header">
        <p class="surface-kicker">Producción y decoración</p>
...
export class ProductionPageComponent implements OnInit {
  ...
  readonly estadosFiltro = ['TODOS', 'PENDIENTE', 'PREPARACION', 'DECORACION', 'EMPAQUE', 'FINALIZADO', 'CANCELADO'];
  ...
  readonly activeQueue = computed(() => 
    this.paginatedProductions().filter(item => item.status !== "FINALIZADO" && item.status !== "CANCELADO")
  );
    ...
    switch (item.status) {
      case "PENDIENTE":
        return {
          ...
          reason: "Inicio de preparación en producción."
        };
      case "PREPARACION":
        return {
          status: "DECORACION",
  ...
  previousAction(item: ProductionSummary): ProductionAction | null {
    // Desde T12 el backend protege avance lineal y cancelacion controlada.
    // Las correcciones operativas deben modelarse luego como reapertura/auditoria,
    // no como retrocesos libres desde la UI.
    return null;
  }
