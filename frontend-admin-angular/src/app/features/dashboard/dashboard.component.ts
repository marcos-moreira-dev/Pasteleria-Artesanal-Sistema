...
export class DashboardComponent {
  ...
  readonly commercialPulse = computed(() => {
    ...
    const activeProduction = production.filter((item) => item.status !== "FINALIZADO" && item.status !== "CANCELADO").length;
  ...
  readonly activeProductionCount = computed(() =>
    this.facade.productionQueue().filter((item) => item.status !== "FINALIZADO" && item.status !== "CANCELADO").length
  );
