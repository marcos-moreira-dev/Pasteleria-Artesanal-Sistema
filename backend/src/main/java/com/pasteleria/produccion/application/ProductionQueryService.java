...
public class ProductionQueryService {
  ...
  public ProductionQueryService(
      ...
      ProductionMaterialMapper productionMaterialMapper,
    ...
    this.productionMaterialMapper = productionMaterialMapper;
  ...
  public List<ProductionConsumptionSummary> listConsumptions(Long productionId) {
    ensureProductionExists(productionId);
    return productionMaterialService.listConsumptions(productionId, productionMaterialMapper);
  }

  public List<ProductionBatchSummary> listBatches(Long productionId) {
    ensureProductionExists(productionId);
    return productionMaterialService.listBatches(productionId, productionMaterialMapper);
  }

  public List<ProductionFinishedEntrySummary> listFinishedEntries(Long productionId) {
    ensureProductionExists(productionId);
    return productionMaterialService.listEntries(productionId, productionMaterialMapper);
  }
  ...
  private void ensureProductionExists(Long productionId) {
    productionRepository.findById(productionId)
        .orElseThrow(() -> new com.pasteleria.common.error.ResourceNotFoundException("El registro de producción indicado no existe."));
  }

  private Comparator<ProductionEntity> buildProductionComparator() {
    return Comparator
        .comparingInt((ProductionEntity production) -> production.getStatus() == ProductionStatus.FINALIZADO || production.getStatus() == ProductionStatus.CANCELADO ? 1 : 0)
