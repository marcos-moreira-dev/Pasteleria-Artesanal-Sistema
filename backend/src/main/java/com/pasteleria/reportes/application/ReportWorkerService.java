  ...
  private GeneratedReport buildBusinessSummary(String jobCode) {
    ...
    byte[] pdf = reportPdfDocumentService.buildBusinessSummaryPdf(
        new ReportPdfDocumentService.BusinessSummaryData(
            ...
            quotations.size(),
            orders.size(),
            productionItems.stream().filter(item -> item.getStatus() != ProductionStatus.FINALIZADO && item.getStatus() != ProductionStatus.CANCELADO).count(),
            deliveredTotal,
            orders.stream()
                .limit(8)
    ...
    byte[] pdf = reportPdfDocumentService.buildProductionQueuePdf(
        new ReportPdfDocumentService.ProductionQueueData(
            ...
            productionRepository.findAllWithOrderAndClient().stream()
                .filter(item -> item.getStatus() != ProductionStatus.FINALIZADO && item.getStatus() != ProductionStatus.CANCELADO)
