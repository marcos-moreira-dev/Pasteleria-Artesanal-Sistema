...
public class RecetaPdfService {
  ...
  public RecetaPdfService(StorageProperties storageProperties) {
  ...
  public byte[] generarRecetaPdf(ProductSummary producto, RecetaJsonDto receta) {
    try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
      ...
      writer.setPageEvent(new PdfWhiteBackgroundPageEvent());
      document.open();
  ...
  private void agregarHeader(Document doc) throws Exception {
    ...
    Image logo = cargarImagen(storageProperties.assetsPath().resolve("branding").resolve("logo-horizontal.png"));
    if (logo == null) {
      logo = cargarImagen(storageProperties.assetsPath().resolve("branding").resolve("logo-cuadrado.png"));
    }
  ...
  private Image cargarImagenProducto(String imagePath) {
    ...
    if (normalized.startsWith("api/v1/assets/")) {
      normalized = normalized.substring("api/v1/assets/".length());
    } else if (normalized.startsWith("assets/")) {
      normalized = normalized.substring("assets/".length());
    } else {
      return null;
    }
    return cargarImagen(storageProperties.assetsPath().resolve(normalized).normalize());
  }
