...
import com.pasteleria.pedidos.domain.service.OrderStateMachine;
...
public class OrderCommandService {
  ...
  public OrderSummary createOrder(CreateOrderRequest request, HttpServletRequest httpRequest) {
    autorizacionService.exigirPermisoSucursal(DEFAULT_SUCURSAL_ID, Permisos.PEDIDOS_GESTIONAR);
  ...
  public OrderSummary updateOrderStatus(Long orderId, UpdateOrderStatusRequest request, HttpServletRequest httpRequest) {
    ...
    orderStateMachine.assertTransitionAllowed(order, request.status());
    ...
    if (request.status() == OrderStatus.CANCELADO && order.getProduction() != null) {
      ...
      if (production.getStatus() != ProductionStatus.FINALIZADO) {
        ...
        production.setProductionNotes(appendOperationalNote(
            production.getProductionNotes(),
            "Produccion cancelada automaticamente por cancelacion del pedido."
        ));
      }
    }
  ...
  private String appendOperationalNote(String currentNotes, String note) {
