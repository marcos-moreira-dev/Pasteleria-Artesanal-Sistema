-- =============================================================================
-- PASTELERIA ERP - V1 BASE ACTUAL COMPACTA
-- Fuente: db/V1/DATABASE_SCHEMA_CANONICO.sql + db/V1/DATABASE_SEED_CANONICO.sql
-- Tanda: T07 - Base de datos V1/V2 y migraciones por ambiente
-- Nota: esta migración compacta reemplaza la línea Flyway legacy V1..V15.
--       La historia legacy se conserva en db/legacy/flyway-history/.
-- =============================================================================
...
-- SECCIÓN 1: LIMPIEZA
-- =============================================================================
DROP TABLE IF EXISTS documento_fiscal CASCADE;
DROP TABLE IF EXISTS asiento_contable_detalle CASCADE;
DROP TABLE IF EXISTS asiento_contable CASCADE;
DROP TABLE IF EXISTS cuenta_contable CASCADE;
DROP TABLE IF EXISTS tipo_diario_contable CASCADE;
...
-- Tabla: tipo_diario_contable
CREATE TABLE tipo_diario_contable (
...
CREATE TABLE cuenta_contable (
    ...
    cuenta_padre_id BIGINT REFERENCES cuenta_contable(cuenta_contable_id),
...
CREATE TABLE asiento_contable (
    ...
    tipo_diario_contable_id BIGINT NOT NULL REFERENCES tipo_diario_contable(tipo_diario_contable_id),
    codigo VARCHAR(60) NOT NULL UNIQUE,
    ...
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    version BIGINT NOT NULL DEFAULT 0,
    CONSTRAINT chk_asiento_contable_cuadre CHECK (total_debe = total_haber),
    CONSTRAINT chk_asiento_contable_origen_completo CHECK (
        (origen_tipo IS NULL AND origen_id IS NULL) OR (origen_tipo IS NOT NULL AND origen_id IS NOT NULL)
    )
);

CREATE UNIQUE INDEX ux_asiento_contable_origen
    ON asiento_contable (origen_tipo, origen_id)
    WHERE origen_tipo IS NOT NULL AND origen_id IS NOT NULL;

-- Tabla: asiento_contable_detalle
CREATE TABLE asiento_contable_detalle (
    asiento_contable_detalle_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    asiento_contable_id BIGINT NOT NULL REFERENCES asiento_contable(asiento_contable_id) ON DELETE CASCADE,
    cuenta_contable_id BIGINT NOT NULL REFERENCES cuenta_contable(cuenta_contable_id),
    descripcion TEXT,
    debe NUMERIC(12,2) NOT NULL DEFAULT 0 CHECK (debe >= 0),
    haber NUMERIC(12,2) NOT NULL DEFAULT 0 CHECK (haber >= 0),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT chk_asiento_detalle_debe_haber_exclusivo CHECK (
...
CREATE TABLE documento_fiscal (
    ...
    documento_cobrar_id BIGINT UNIQUE REFERENCES documento_cobrar(documento_cobrar_id),
    documento_compra_id BIGINT UNIQUE REFERENCES documento_compra(documento_compra_id),
    ...
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    version BIGINT NOT NULL DEFAULT 0,
    CONSTRAINT chk_documento_fiscal_origen_unico CHECK (
    ...
    CONSTRAINT chk_documento_fiscal_total CHECK (subtotal + impuesto = total),
    CONSTRAINT chk_documento_fiscal_tercero CHECK (
        (tercero_tipo = 'CLIENTE' AND documento_cobrar_id IS NOT NULL) OR
        (tercero_tipo = 'PROVEEDOR' AND documento_compra_id IS NOT NULL)
    )
);
...
CREATE INDEX idx_cuenta_contable_activa ON cuenta_contable(activa, codigo);
CREATE INDEX idx_asiento_contable_fecha ON asiento_contable(fecha_asiento DESC);
CREATE INDEX idx_asiento_contable_origen ON asiento_contable(origen_tipo, origen_id);
CREATE INDEX idx_asiento_detalle_asiento ON asiento_contable_detalle(asiento_contable_id);
CREATE INDEX idx_asiento_detalle_cuenta ON asiento_contable_detalle(cuenta_contable_id);
CREATE INDEX idx_documento_fiscal_estado ON documento_fiscal(estado, fecha_emision DESC);
CREATE INDEX idx_documento_fiscal_tercero ON documento_fiscal(tercero_tipo, tercero_id);
CREATE INDEX idx_documento_fiscal_origen_cobrar ON documento_fiscal(documento_cobrar_id);
CREATE INDEX idx_documento_fiscal_origen_compra ON documento_fiscal(documento_compra_id);
...
INSERT INTO tipo_diario_contable (codigo, nombre, descripcion, activo, created_at) VALUES
('VENTAS', 'Diario de ventas', 'Asientos internos relacionados con ventas y cartera.', TRUE, NOW()),
('COMPRAS', 'Diario de compras', 'Asientos internos relacionados con compras y cuentas por pagar.', TRUE, NOW()),
('CAJA', 'Diario de caja', 'Asientos internos relacionados con caja, cobros y pagos.', TRUE, NOW()),
('GENERAL', 'Diario general', 'Asientos manuales y ajustes internos controlados.', TRUE, NOW())
ON CONFLICT (codigo) DO NOTHING;

INSERT INTO cuenta_contable (codigo, nombre, tipo_cuenta, naturaleza, nivel, cuenta_padre_id, imputable, activa, created_at, updated_at, version) VALUES
('1', 'Activo', 'ACTIVO', 'DEUDORA', 1, NULL, FALSE, TRUE, NOW(), NOW(), 0),
('1.1', 'Caja y bancos', 'ACTIVO', 'DEUDORA', 2, NULL, FALSE, TRUE, NOW(), NOW(), 0),
('1.1.01', 'Caja principal', 'ACTIVO', 'DEUDORA', 3, NULL, TRUE, TRUE, NOW(), NOW(), 0),
('1.2', 'Cuentas por cobrar', 'ACTIVO', 'DEUDORA', 2, NULL, FALSE, TRUE, NOW(), NOW(), 0),
('1.2.01', 'Clientes nacionales', 'ACTIVO', 'DEUDORA', 3, NULL, TRUE, TRUE, NOW(), NOW(), 0),
('2', 'Pasivo', 'PASIVO', 'ACREEDORA', 1, NULL, FALSE, TRUE, NOW(), NOW(), 0),
('2.1', 'Cuentas por pagar', 'PASIVO', 'ACREEDORA', 2, NULL, FALSE, TRUE, NOW(), NOW(), 0),
('2.1.01', 'Proveedores nacionales', 'PASIVO', 'ACREEDORA', 3, NULL, TRUE, TRUE, NOW(), NOW(), 0),
('3', 'Patrimonio', 'PATRIMONIO', 'ACREEDORA', 1, NULL, FALSE, TRUE, NOW(), NOW(), 0),
('3.1.01', 'Capital inicial', 'PATRIMONIO', 'ACREEDORA', 2, NULL, TRUE, TRUE, NOW(), NOW(), 0),
('4', 'Ingresos', 'INGRESO', 'ACREEDORA', 1, NULL, FALSE, TRUE, NOW(), NOW(), 0),
('4.1.01', 'Ventas de pasteleria', 'INGRESO', 'ACREEDORA', 2, NULL, TRUE, TRUE, NOW(), NOW(), 0),
('5', 'Costos', 'COSTO', 'DEUDORA', 1, NULL, FALSE, TRUE, NOW(), NOW(), 0),
('5.1.01', 'Costo de insumos de produccion', 'COSTO', 'DEUDORA', 2, NULL, TRUE, TRUE, NOW(), NOW(), 0),
('6', 'Gastos', 'GASTO', 'DEUDORA', 1, NULL, FALSE, TRUE, NOW(), NOW(), 0),
('6.1.01', 'Gastos operativos generales', 'GASTO', 'DEUDORA', 2, NULL, TRUE, TRUE, NOW(), NOW(), 0)
ON CONFLICT (codigo) DO NOTHING;

UPDATE cuenta_contable hijo SET cuenta_padre_id = padre.cuenta_contable_id
FROM cuenta_contable padre
WHERE hijo.codigo LIKE padre.codigo || '.%'
  AND hijo.cuenta_padre_id IS NULL
  AND length(hijo.codigo) - length(replace(hijo.codigo, '.', '')) = length(padre.codigo) - length(replace(padre.codigo, '.', '')) + 1;
